...
describe("syria", () => {
	describe("LOtoLL", () => {
		test("x too small", () => {
			expect(() => LOtoLL({ map: "syria", x: -340_001, z: 0 })).toThrow(
				new Error("Coordinates outside of acceptable area for syria")
			);
		});

		test("x too big", () => {
			expect(() => LOtoLL({ map: "syria", x: 310_000, z: 0 })).toThrow(
				new Error("Coordinates outside of acceptable area for syria")
			);
		});

		test("z too small", () => {
			expect(() => LOtoLL({ map: "syria", x: 0, z: -410_001 })).toThrow(
				new Error("Coordinates outside of acceptable area for syria")
			);
		});

		test("z too big", () => {
			expect(() => LOtoLL({ map: "syria", x: 0, z: 450_000 })).toThrow(
				new Error("Coordinates outside of acceptable area for syria")
			);
		});

		test("corner NE", () => {
			const { lat, lng } = LOtoLL({ map: "syria", x: 309_999, z: 449_999 });
			expect(lat).toBeCloseTo(37.84091, 5);
			expect(lng).toBeCloseTo(40.90018, 5);
		});

		test("corner SE", () => {
			const { lat, lng } = LOtoLL({ map: "syria", x: -340_000, z: 449_999 });
			expect(lat).toBeCloseTo(31.98252, 5);
			expect(lng).toBeCloseTo(40.76967, 5);
		});

		test("corner SW", () => {
			const { lat, lng } = LOtoLL({ map: "syria", x: -340_000, z: -410_000 });
			expect(lat).toBeCloseTo(31.78417, 5);
			expect(lng).toBeCloseTo(31.69127, 5);
		});

		test("corner NW", () => {
			const { lat, lng } = LOtoLL({ map: "syria", x: 309_999, z: -410_000 });
